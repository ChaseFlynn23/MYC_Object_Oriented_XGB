...
class XGBoostTrainer:
    ...
    def calculate_and_plot_positions(self, importance_thresholds):
        """
        Calculate and plot the number of positions saved for different importance_thresholds
        using the pre-generated feature importance CSV files. Ensure all plots have standardized axes,
        correctly formatted titles, and now includes a plot of the total number of positions above
        each threshold. New positions above the threshold are colored in red, and positions that were
        present in the previous graphs are colored in blue.
        """
        # Initialize a dictionary to track the total number of positions for each threshold
        total_positions_by_threshold = {}
        previous_covered_positions = set()  # Initialize an empty set for tracking previously covered positions
        ...
        for threshold in importance_thresholds:
            ...
            for window_size in self.window_sizes:
                ...
                if not df_importances.empty:
                    ...
                    total_positions += len(important_features)  # Add count of important features

                    # Calculate residues covered for the window size
                    for feature in important_features:
                        ...
                        # Determine color based on whether the position was covered previously
            ...
            # Update the set of previously covered positions for the next iteration
            previous_covered_positions = current_covered_positions.copy()

            # Store the total positions count for the current threshold
            total_positions_by_threshold[threshold] = total_positions

            # Correctly format the threshold value in the title
            formatted_threshold = f"{threshold:.3f}"
            plt.title(f'Residues Covered for Threshold {formatted_threshold}')
    ...
    def save_positions_above_threshold(self, importance_threshold):
        """
        Save positions above the specified importance_threshold for all window sizes
        using the pre-generated feature importance CSV files.
        """
        total_positions_saved = 0
        for window_size in self.window_sizes:
            adjusted_threshold = self.calculate_adjusted_threshold(importance_threshold, window_size)
            df_importances = self.read_feature_importances(window_size)
            ...
            if not df_importances.empty:
                ...
                if len(important_features) > 0:
                    print(f"Window size {window_size}: {len(important_features)} positions saved. Positions: {important_features.tolist()}")
